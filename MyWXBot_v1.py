    ...
    def handle_msg_all(self, msg):
		...
		if msg['msg_type_id'] == 3 and msg['content']['type'] == 0:  # group text message
			...
			if msg['content']['data'] == u'汇总':
				if msg['content']['user']['name'] == 'AndersenChen':
					mydict = {}
					baodan_records = ''
					...
					self.send_msg_by_uid(u'该功能被限制', msg['user']['id'])
					
			
			elif msg['content']['data'] == u'明细':
				
				all_records = ''
				reply = ''
				with open('baodan.txt', 'r') as a_file:
					all_records = a_file.read()
					#print(all_records.decode(encoding='UTF-8'))
					
				self.send_msg_by_uid(all_records, msg['user']['id'])	
